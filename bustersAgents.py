...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for item in livingGhostPositionDistributions:
      ...
      currentProb=0
      for i in item:
        if item[i]>currentProb:
          currentLocation=i
          currentProb=item[i]
      if self.distancer.getDistance(currentLocation,pacmanPosition)<closestDistance:
        closestDistance=self.distancer.getDistance(currentLocation,pacmanPosition)
    ...
    for item in legal:
      if item!="Stop":
        bestmove=item
        break
    minDistance=self.distancer.getDistance(pacmanPosition,closestLocation)
    for action in legal:
      successorPosition = Actions.getSuccessor(pacmanPosition, action)
      if self.distancer.getDistance(successorPosition,closestLocation)<minDistance:
        minDistance=self.distancer.getDistance(successorPosition,closestLocation)
        bestMove=action
    return bestMove
