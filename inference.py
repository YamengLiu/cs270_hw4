...
class ExactInference(InferenceModule):
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    allPossible=util.Counter()
    for eachGhostPosition in self.legalPositions:
      newPosDist=self.getPositionDistribution(self.setGhostPosition(gameState, eachGhostPosition))
      for newPos, prob in newPosDist.items():
        allPossible[newPos]+=self.beliefs[eachGhostPosition]*prob
...
class MarginalInference(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    jointInference.addGhostAgentmaanhattan(self.ghostAgent)
